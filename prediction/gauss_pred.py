...
def print_loo_and_ks(samples):
    ...
    ks_sum = [[
        (ks <= 0.5).sum(),
        sum([1 for k in ks if 0.5 < k <= 0.7]),
        (ks > 0.7).sum()
    ]]
...
run_linear_model('prediction/gaussian_model.stan',
                 'Gaussian linear prediction',
                 'predicting_big_coins_gauss.png')

run_linear_model('prediction/student_t_model.stan',
                 'Student\'s t linear prediction',
                 'predicting_big_coins_t.png')
